...
```{r}
UniFish = ordinate(fishy.r, method = "PCoA", distance = "unifrac")
UniFish$vectors[,1:4]
fishdr = as.data.frame(sample_data(fishy.r))
...
(0.3645994+0.4299988)/2

x=0.3972991/0.4299988

y=0.3972991/0.3645994
...
mean(Day0fed$logV) #1.298002
mean(Day0starved$logV) #1.157416
...
p=ggplot(AlphaPlot2, aes(x=ExptlTimePoint, y=logVt)) + geom_point(aes(color=FedStarved), alpha = 0.3) + scale_color_manual(breaks=c("fed","starved"),values=c("#bf812d", "#35978f")) +stat_summary(aes(y=logVt, group = FedStarved), fun.data = mean_se, geom = "errorbar", width = 0.1)+stat_summary(aes(y=logVt, group = FedStarved, color = FedStarved), fun.y = mean, geom = "line")+theme_bw()+xlab("Experimental Time Point")+ylab("Normalized log Alpha Diversity (Faith's PD)")
p
```
